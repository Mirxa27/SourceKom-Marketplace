...
export async function POST(request: NextRequest) {
  try {
    ...
    // Create admin/global notification
    try {
      await db.notification.create({
        data: {
          title: `New ${inquiry.type.replace('_', ' ')} inquiry`,
          message: `${inquiry.name} (${inquiry.email}) submitted a new inquiry`,
          type: 'system',
          isGlobal: true,
          actionUrl: `/admin/inquiries?id=${inquiry.id}`,
          actionText: 'Review inquiry'
        }
      })
    } catch (notifyErr) {
      console.warn('Failed to create admin notification for inquiry', notifyErr)
    }
